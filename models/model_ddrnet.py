...
class ModelDDRNet(ModelPlain):
    """Train video restoration  with pixel loss"""
    def __init__(self, opt):
        super(ModelDDRNet, self).__init__(opt)
    ...
    def define_optimizer(self):
        ...
        if self.opt_train.get('fix_iter', 0) and len(self.fix_keys) > 0:
            ...
            super(ModelDDRNet, self).define_optimizer()
    ...
    def optimize_parameters(self, current_step):
        ...
        super(ModelDDRNet, self).optimize_parameters(current_step)
            ...
            for h_idx in h_idx_list:
                for w_idx in w_idx_list:
                    ...
                    if hasattr(self, 'netE'):
                        out_patch,_,_,_ = self.netE(in_patch)
                        out_patch =out_patch.detach().cpu()
                    else:
                        out_patch,_,_,_ = self.netG(in_patch)
                        out_patch =out_patch.detach().cpu()
